...
def auth(message):
    with sqlite3.connect("../DataBase/university.db") as connection:
        mycursor=connection.cursor()
        if student_id.canLogin:
            if not student_id.auth:
                s_id = message.text[1:6]
                sql = '''select password from student where id="{0}"'''.format(s_id)
...
def get_hrefs(message):
    with sqlite3.connect("../DataBase/university.db") as connection:
        ...
        if student_id.auth:
            buttons = []
            sql = '''select title,course_id from course where dept_name=(select dept_name from student where id="{0}")'''.format(student_id.staticVariable)
            ...
            if len(myresult) != 0:
                for row in myresult:
                    ...
                    bot.send_message(message.chat.id,
                                     "Title: {0:20}       Year: {1}       Grade:    {2:2}".format(title, row[1],
                                                                                                  row[2]))
            else:
    ...
    with sqlite3.connect("../DataBase/university.db") as connection:
        ...
        if student_id.auth:
            if "$$" in str(query.data):
                ...
                sql = '''select course_id,sec_id,semester,year from section where course_id = "{0}"'''.format(str(query.data)[2:9])
                ...
                if sec_exist and not isTake:
                    ...
                    # sql_id = (st_id)
                    mycursor.execute(
                        '''select * from takes where id="{0}"'''.format(st_id))
                    myresult = mycursor.fetchall()
                    buttons = []
                    keyboard = types.InlineKeyboardMarkup()
                    for row in myresult:
                    ...
                    keyboard.add(*buttons)
                    bot.reply_to(query.message,"takes ", reply_markup=keyboard)
                    del keyboard
                    # del sql
                    # del course_id
                    # del buttons
